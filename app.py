...
@st.cache_resource
def load_artifacts():
    model    = joblib.load("Customer_churn_logistic_model.joblib")
    scaler   = joblib.load("Customer_churn_scaler.joblib")
    encoders = joblib.load("Customer_churn_encoders.joblib")
    features = joblib.load("Customer_churn_feature_names.joblib")
    with open("metrics.json") as f:
        results = json.load(f)
    return model, scaler, encoders, features, results

model, scaler, encoders, FEATURE_NAMES, results = load_artifacts()
# After model, scaler, encoders, FEATURE_NAMES, results = load_artifacts()
# Add this (approx line 30-35)

if 'prediction_made' not in st.session_state:
    st.session_state.prediction_made = False
    st.session_state.prob = 0.5          # default placeholder
...
/* Force dark background on all columns */
div[data-testid="column"] {
    background-color: transparent !important;
}

/* Force dark background on all blocks */
div[data-testid="stBlock"] {
    background-color: transparent !important;
}

/* Force dark background on all horizontal blocks */
div[data-testid="stHorizontalBlock"] {
    background-color: transparent !important;
}

/* Force dark background on all vertical blocks */
...
/* ── Section labels (Account Info, Billing, etc.) ── */
.section-label {
...
div[data-baseweb="select"] > div {
  ...
  color: #000000 !important;  /* This makes dropdown text black */
...
.stTextInput input {
  background: #ffffff !important;
  color: #000000 !important;  /* This makes input text black */
  font-weight: 800 !important;
  border: 1px solid #cccccc !important;
}
...
with left:
    ...
    c3, c4 = st.columns(2)
    with c3:
        monthly = st.number_input("Monthly Charges ($)", 20.0, 150.0, 78.5, step=0.5, key="mc")
    with c4:
        total = st.number_input("Total Charges ($)", 0.0, 15000.0, float(round(tenure * monthly, 2)), step=1.0,
                                key="tc")

    # ── Plan details ──
    st.markdown('<div class="section-label">Plan Details</div>', unsafe_allow_html=True)

    # Remove the custom HTML labels and let Streamlit show its own labels
    contract = st.selectbox("Contract Type", ["Month-to-month", "One year", "Two year"], key="ct")
    ...
    c5, c6 = st.columns(2)
    with c5:
        tech_support = st.selectbox("Tech Support", ["No", "Yes"], key="ts")
    with c6:
        online_security = st.selectbox("Online Security", ["No", "Yes"], key="os")
...
# ── Helper: LabelEncode inputs → scale → predict ──────────────────────────────
    def update_prediction():
        """Calculate prediction based on current input values and store in session state"""
        row = {
            'Age': st.session_state.age,
            'Tenure': st.session_state.tenure,
            'MonthlyCharges': st.session_state.mc,
            'TotalCharges': st.session_state.tc,
            'CustomerServiceCalls': st.session_state.csc,
            'DataUsageGB': st.session_state.dg,
            'ContractType': encoders['ContractType'].transform([st.session_state.ct])[0],
            'PaymentMethod': encoders['PaymentMethod'].transform([st.session_state.pm])[0],
            'InternetService': encoders['InternetService'].transform([st.session_state.internet])[0],
            'TechSupport': encoders['TechSupport'].transform([st.session_state.ts])[0],
            'OnlineSecurity': encoders['OnlineSecurity'].transform([st.session_state.os])[0],
        }
        X_in = pd.DataFrame([row])[FEATURE_NAMES]
        X_sc = scaler.transform(X_in)
        st.session_state.prob = model.predict_proba(X_sc)[0][1]
        pred = int(st.session_state.prob >= 0.5)


        st.session_state.pred = pred
        st.session_state.feature_row = row
...
with right:
    ...
    with hcol1:
        pct = int(st.session_state.prob * 100)
        bar_col = f"linear-gradient(90deg, {'#f43f5e' if st.session_state.pred else '#10b981'}, {'#fb7185' if st.session_state.pred else '#34d399'})"
        cls = "churn" if st.session_state.pred else "no-churn"
        icon = "🚨" if st.session_state.pred else "✅"
        label = "WILL CHURN" if st.session_state.pred else "WILL STAY"
    ...
    with hcol2:
        # Live gauge chart
        gauge_col = "#f43f5e" if st.session_state.pred else "#10b981"
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number+delta",
            value=round(st.session_state.prob * 100, 1),
            ...
            gauge={
                ...
                "threshold": {
                    ...
                    "thickness": 0.75, "value": st.session_state.prob * 100
                },
            },
            domain={"x": [0, 1], "y": [0, 1]},
        ))
    ...
    st.markdown(f"""
        <div style="background: #0d1424; border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; padding: 1.5rem; font-family: 'Space Grotesk', sans-serif; color: #e2e8f0; max-width: 100%; margin: 0 auto;">

          <!-- Account Info -->
          <div style="margin-bottom: 1.5rem;">
            <div style="font-size: 1.1rem; font-weight: 700; color: #ffffff; margin-bottom: 1rem; letter-spacing: 0.02em; display: flex; align-items: center; gap: 0.4rem;">
              <span style="width: 3px; height: 16px; background: #6366f1; border-radius: 2px; display: inline-block;"></span>
              Account Info
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Age</div><div style="font-size: 1.1rem; font-weight: 600;">{age}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Service Calls</div><div style="font-size: 1.1rem; font-weight: 600;">{csc}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Tenure (months)</div><div style="font-size: 1.1rem; font-weight: 600;">{tenure}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Data Usage (GB)</div><div style="font-size: 1.1rem; font-weight: 600;">{data_usage:.2f}</div></div>
            </div>
          </div>
          ...
          <div style="margin-bottom: 1.5rem;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Monthly Charges ($)</div><div style="font-size: 1.1rem; font-weight: 600;">{monthly:.2f}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Total Charges ($)</div><div style="font-size: 1.1rem; font-weight: 600;">{total:.2f}</div></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.75rem;">
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Contract Type</div><div style="font-size: 1rem; font-weight: 500;">{contract}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Payment Method</div><div style="font-size: 1rem; font-weight: 500;">{payment}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Internet Service</div><div style="font-size: 1rem; font-weight: 500;">{internet}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Tech Support</div><div style="font-size: 1rem; font-weight: 500;">{tech_support}</div></div>
              <div><div style="font-size: 0.75rem; color: #4b5563; text-transform: uppercase; letter-spacing: 0.06em;">Online Security</div><div style="font-size: 1rem; font-weight: 500;">{online_security}</div></div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with tab1:
        ...
        # Get the current feature row, or create a default one if none exists
        if st.session_state.feature_row is not None:
            current_row = st.session_state.feature_row
        else:
            # Create a default row from current input values
            current_row = {
                'Age': st.session_state.age,
                'Tenure': st.session_state.tenure,
                'MonthlyCharges': st.session_state.mc,
    ...
    with tab3:
        ...
        fig_dist.add_vline(
            x=st.session_state.prob * 100,
            line_color="#fbbf24", line_width=2, line_dash="dot",
            annotation_text=f"  This customer: {st.session_state.prob*100:.1f}%",
    ...
    def scenario_prob(contract_ov=None, tech_ov=None, sec_ov=None,
        ...
        # Get the current feature row, or create a default one
        if st.session_state.feature_row is not None:
            r = {**st.session_state.feature_row}
        else:
            r = {
                'Age': st.session_state.age,
                'Tenure': st.session_state.tenure,
        ...
        c = contract_ov or st.session_state.ct
        t = tech_ov or st.session_state.ts
        s = sec_ov or st.session_state.os
        m = monthly_ov if monthly_ov is not None else st.session_state.mc
    ...
    # Current
    current_p = st.session_state.prob
    ...
    if st.session_state.pred:
        ...
        <div class="card" style="border-color:rgba(16,185,129,0.2);background:rgba(16,185,129,0.04)">
          ...
          <p style="font-size:0.85rem;color:#94a3b8;line-height:1.7;margin:0">
            This customer has a <strong style="color:#10b981">low churn risk of {st.session_state.prob*100:.1f}%</strong>.
