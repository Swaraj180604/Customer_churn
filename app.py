...
# ── Page Config ────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ChurnRadar · AI Prediction",
    page_icon="🎯",
    layout="wide",
...
def load_artifacts():
    ...
    feature_names = joblib.load("Customer_churn_feature_names.joblib")
...
results = {
    ...
    "roc_auc": results.get("ROC AUC", 0),
}
FEATURE_IMPORTANCE = dict(
    zip(FEATURE_NAMES, abs(model.coef_[0]))
)
...
/* ── Left panel ── */
/* Make left panel more vibrant */
.left-panel {
  background: linear-gradient(145deg, #0f1a2f 0%, #0a1120 100%);
  border-right: 1px solid rgba(99, 102, 241, 0.3);
}

/* Add section labels that stand out */
.section-label {
...
<div class="topbar">
  <div class="logo">
    ...
    ChurnRadar
    <span style="color:#4b5563;font-weight:400;font-size:0.85rem">· AI Prediction Engine</span>
  </div>
  <div class="topbar-badges">
    ...
    <span class="topbar-badge">15 features</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
with left:
    ...
    with c4:
        total   = st.number_input("Total Charges ($)", 0.0, 15000.0, float(round(tenure * monthly, 2)), step=1.0, key="tc")
    ...
    payment   = st.selectbox("Payment Method", ["Bank transfer", "Credit card", "Electronic check", "Mailed check"], key="pm")
    internet  = st.selectbox("Internet Service", ["DSL", "Fiber optic", "No"], key="is")
...
with right:
    ...
    <div class="stat-row">
      <div class="stat-box">
        <div class="stat-val" style="color:#6366f1">{results['accuracy']*100:.1f}%</div>
        <div class="stat-lbl">Accuracy</div>
      </div>
      <div class="stat-box">
        <div class="stat-val" style="color:#8b5cf6">{results['precision']*100:.1f}%</div>
        <div class="stat-lbl">Precision</div>
      </div>
      <div class="stat-box">
        <div class="stat-val" style="color:#06b6d4">{results['recall']*100:.1f}%</div>
        <div class="stat-lbl">Recall</div>
      </div>
      <div class="stat-box">
        <div class="stat-val" style="color:#10b981">{results['f1']*100:.1f}%</div>
        <div class="stat-lbl">F1 Score</div>
      </div>
      <div class="stat-box">
        <div class="stat-val" style="color:#f59e0b">{results['roc_auc']*100:.1f}%</div>
    ...
    tab1, tab2, tab3 = st.tabs(["📈  Sensitivity Analysis", "🎯  Feature Impact", "📊  Score Distribution"])
    ...
    # ── Tab 2: Feature impact for this specific customer ──────────
    with tab2:
        st.markdown('<div style="height:0.5rem"></div>', unsafe_allow_html=True)

        # Weighted impact: feature value * importance
        impacts = {}
        for fname in FEATURE_NAMES:
            val = feature_row[fname]
            imp = FEATURE_IMPORTANCE.get(fname, 0)
            impacts[fname] = val * imp

        # Sort by absolute value
        imp_sorted = sorted(FEATURE_IMPORTANCE.items(), key=lambda x: x[1], reverse=True)
        top_features = imp_sorted[:10]

        feat_labels = [f[0].replace("_", " ").replace("ContractType ", "").replace("PaymentMethod ", "").replace("InternetService ", "") for f, _ in top_features]
        feat_vals = [f * 100 for _, f in top_features]
        ...
        fig_imp = go.Figure(go.Bar(
            ...
            marker=dict(
                ...
                colorbar=dict(
                    ...
                    title=dict(text="Importance", font_color=TICKCOL, side="right"),
                )
            ),
        ...
        fig_imp.update_layout(
            paper_bgcolor=PLOT_PAP, plot_bgcolor=PLOT_BG,
            xaxis=dict(tickfont_color=TICKCOL, gridcolor=GRIDCOL,
                       title_text="Feature importance (%)", title_font_color=TICKCOL),
            yaxis=dict(tickfont_color="#94a3b8", gridcolor="rgba(0,0,0,0)"),
    ...
    def scenario_prob(overrides: dict) -> float:
        r = {**feature_row}
        # Reset all categorical flags
        for f in FEATURE_NAMES:
            if f.startswith("ContractType_") or f.startswith("PaymentMethod_") \
               or f.startswith("InternetService_") or f in ["TechSupport_Yes", "OnlineSecurity_Yes"]:
                r[f] = 0
        r.update(overrides)
    ...
    r_a = {**feature_row, "ContractType_One year": 0, "ContractType_Two year": 1}
    p_a = scenario_prob({"ContractType_Two year": 1, "ContractType_One year": 0})

    # Scenario B: add tech support + security
    r_b = {**feature_row, "TechSupport_Yes": 1, "OnlineSecurity_Yes": 1}
    p_b = scenario_prob({"TechSupport_Yes": 1, "OnlineSecurity_Yes": 1,
                         "ContractType_One year": feature_row.get("ContractType_One year", 0),
                         "ContractType_Two year": feature_row.get("ContractType_Two year", 0)})

    # Scenario C: reduce monthly charges by 20%
    r_c = {**feature_row, "MonthlyCharges": monthly * 0.8}
    Xc  = scaler.transform(pd.DataFrame([r_c])[FEATURE_NAMES])
    p_c = model.predict_proba(Xc)[0][1]
