...
with left:
    ...
    with c2:
        ...
        data_usage = st.slider("Data Usage Monthly Average(GB)", 0.0, 500.0, 35.0, step=0.5, key="dg")
...
with right:
    ...
    # ── Show results only after first prediction ────────────────────
    if st.session_state.prediction_made:
        ...
        with tab1:
            ...
            for fname, (lo, hi, cur) in continuous_features.items():
                hi_row = {**current_row}; hi_row[fname] = hi
                lo_row = {**current_row}; lo_row[fname] = lo
        ...
        with tab2:
            ...
            feat_vals   = [a[1] * 100 for a in abs_sorted]
        ...
        st.markdown("""
        <div class="card" style="text-align: center; padding: 3rem 2rem; border-color: #2d3748;">
            <div style="font-size: 3rem; margin-bottom: 1rem; opacity: 0.5;">🔮</div>
            <div style="font-size: 1.2rem; font-weight: 600; color: #a0aec0;">Ready to analyze churn risk</div>
            <div style="font-size: 0.9rem; color: #4a5568; margin-top: 0.5rem;">
                Adjust the inputs on the left and click<br>
                <span style="font-weight: 600; color: #6366f1;">⚡ Run Prediction</span> to see results.
            </div>
        </div>
        """, unsafe_allow_html=True)